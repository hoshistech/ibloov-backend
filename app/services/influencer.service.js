...
module.exports = {
    ...
    all: async ( query, options ) =>{
        ...
        .limit(limit)
        .skip(skip)
        .populate('userId', '_id avatar authMethod email local.firstName local.lastName google facebook apple fullName')
        .populate('events.eventId', '_id name startDate')
        .populate('followers.userId', '_id avatar authMethod email local.firstName local.lastName google facebook apple fullName')
    ...
    viewInfluencer: async ( influencerId ) => {
        ...
        .populate('userId', '_id avatar authMethod email bio local.firstName local.lastName google facebook apple fullName');
        return influencer;
    },
